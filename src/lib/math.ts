...
export const roundTo3Digits = (num: number) => Math.round(num * 1000) / 1000;

const byDate: Ord<Data> = pipe(
  date.Ord,
  ord.contramap((data: Data) => data.date),
);

export const weightFun = (x: number) => (Math.log(x * 2 + 0.3) / 3 + 5 / 7) ** (1 / 4);

export const filterTop80 = (data: Data[]) => pipe(data, array.splitAt(Math.floor(data.length * 0.2)), tuple.snd);
